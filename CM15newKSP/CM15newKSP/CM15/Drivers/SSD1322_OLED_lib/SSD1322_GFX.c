 ...
 */
void draw_bitmap_8bpp(uint8_t *frame_buffer, const uint16_t *bitmap, uint16_t x0, uint16_t y0, uint16_t x_size, uint16_t y_size)
{
 ...
 *             brightness value of pixels (range 0-15 dec or 0x00-0x0F hex)
 */
void draw_text(uint8_t *frame_buffer, uint8_t* text, uint16_t x, uint16_t y, uint8_t brightness)
{
    while (*text)
    {
        draw_char(frame_buffer, *text, x, y, brightness);
        x = x + gfx_font->glyph[*text-32].xAdvance + 1;
        text++;
    }
}
