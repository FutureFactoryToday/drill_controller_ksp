/*
	...
	1. Add parameters to struct "stored_params_t" (file - FlashParam.h) between 
	startLoadFlag and endLoadFlag
	2. Change Flash size in "Options for Target 'KSP_SPO'->Target" IROM1  and 
	"Options for Target 'KSP_SPO'->Debug->Settings->Flash Download" for	0x3F800
	3. Call function FP_GetParam() to load from flash.
	4. If load from flash is successfully, then fp->isLoaded will be "true".
	5. Modifie parameters in fp->params, set fp->needToSave = true 
	and call FP_SaveParam() to save into flash.
*/
