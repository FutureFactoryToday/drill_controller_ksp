...
//  #endif
 //mmodbus_delay(1); 
	//LL_GPIO_ResetOutputPin(_MMODBUS_CTRL_GPIO, _MMODBUS_CTRL_PIN);
	mmodbus.txDmaDone = 1;
}
//##################################################################################################
uint16_t mmodbus_receiveRaw(uint32_t timeout){
	mmodbus.rxBusy = 1;
	if (HAL_UART_Receive(_MMODBUS_USART,  mmodbus.rxBuf, _MMODBUS_RXSIZE, timeout) != HAL_OK){
		mmodbus.rxIndex = _MMODBUS_RXSIZE - *_MMODBUS_USART.RxXferCount;
	}
//  uint32_t startTime = HAL_GetTick();
//  while(mmodbus.rxBusy == 1)
//  {
//    if(HAL_GetTick() - startTime > timeout){
//			HAL_UART_AbortReceive(_MMODBUS_USART);
//      return 0;
//		}
//	}
//    if ((mmodbus.rxIndex > 0) && (LL_USART_IsActiveFlag_IDLE(_MMODBUS_USART)))
//		mmodbus.rxIndex = size;
...
bool mmodbus_sendRaw(uint8_t *data, uint16_t size, uint32_t timeout)
{
  ...
  mmodbus.rxIndex = 0;
  uint32_t startTime = HAL_GetTick();
  LL_GPIO_SetOutputPin(_MMODBUS_CTRL_GPIO, _MMODBUS_CTRL_PIN);
  mmodbus_delay(1);
  #if (_MMODBUS_TXDMA == 0)
...
//  LL_DMA_EnableChannel(_MMODBUS_DMA, _MMODBUS_DMASTREAM);
//  LL_USART_ClearFlag_TC(_MMODBUS_USART);
HAL_UART_Transmit_DMA(_MMODBUS_USART,data, size);
  while(mmodbus.txDmaDone == 0)
  {
    mmodbus_delay(1);
    if(HAL_GetTick() - startTime > timeout)
    {
			HAL_UART_AbortTransmit(_MMODBUS_USART);
      LL_GPIO_ResetOutputPin(_MMODBUS_CTRL_GPIO, _MMODBUS_CTRL_PIN);
      mmodbus.txBusy = 0;
      return false;
    }  
...
bool mmodbus_init(uint32_t timeout){
  LL_GPIO_ResetOutputPin(_MMODBUS_CTRL_GPIO, _MMODBUS_CTRL_PIN);
	HAL_UART_RegisterCallback(_MMODBUS_USART, HAL_UART_TX_COMPLETE_CB_ID, (pUART_CallbackTypeDef)mmodbus_callback_txDMA);
	HAL_UART_RegisterCallback(_MMODBUS_USART, HAL_UART_RX_COMPLETE_CB_ID, (pUART_CallbackTypeDef)mmodbus_callback);
