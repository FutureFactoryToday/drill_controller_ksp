...
/*
	******************************************************************************
	* @brief	 ( описание ):  вывод буффера зоны кадра на дисплей
	* @param	( параметры ):	координаты зоны
	* @return  ( возвращает ):	

	******************************************************************************
*/
void ST7565_UpdateArea (int16_t xPos, int16_t yPos, int16_t xSize, int16_t ySize){
	...
	ST7565_Select();
  	...
  	for(uint8_t y = 0; y < 8; y++) {
		...
		for(uint8_t x = 0; x < 128; x++) {
            ...
            if((x >= xPos && x <= (xPos + xSize)) && ((y * 8) >= yPos && (y * 8) <= (yPos + ySize))) {
                ST7565_w_dat( ST7565_buffer[x + 128 * y] );
            }
		}
	}
...
void ST7565_DrawFillRoundRect(int16_t x, int16_t y, uint16_t width, uint16_t height, int16_t cornerRadius, uint8_t color) {
  ...
  ST7565_DrawFilledRectangle(x + cornerRadius, y, x + cornerRadius + width - 2 * cornerRadius, y + height, color);
  // draw four corners
...
void ST7565_DrawCircleHelper(int16_t x0, int16_t y0, int16_t radius, int8_t quadrantMask, uint8_t color)
{
    ...
    while (x <= y) {
        ...
        if (quadrantMask & 0x4) {
            ST7565_Draw_pixel(x0 + x, y0 + y, color);
            ST7565_Draw_pixel(x0 + y, y0 + x, color);;
        }
        if (quadrantMask & 0x2) {
			ST7565_Draw_pixel(x0 + x, y0 - y, color);
            ST7565_Draw_pixel(x0 + y, y0 - x, color);
